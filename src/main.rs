...
fn test_pbo_upload<C: glow::HasContext>(gl: &C) {
    ...
    let texture_size = (512, 512, 4);
    let bytes_per_texel = 4i32;
    let copy_size = (44i32, 44i32);
    let copy_origin = (384, 320, texture_size.2 - 1);
    let pbo_offset = 268288i32;
    ...
    unsafe {
        ...
        gl.pixel_store_i32(glow::UNPACK_ROW_LENGTH, 256 / bytes_per_texel);
    ...
    report("copy 1 texel at origin", texels[0], FILL);
    let description = format!("copy {:?} at {:?} by offset {}", copy_size, copy_origin, pbo_offset);
